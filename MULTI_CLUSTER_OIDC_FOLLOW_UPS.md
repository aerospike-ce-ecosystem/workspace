...
- **이유**: multi-cluster 검증 중 발견. ACKO `Makefile`이 단일 `Dockerfile`의 `runtime` target (api+ui 통합, ui가 `/app/ui/public/`에 nest)을 ui.web image로 build해서 chart mountPath(`/app/public/`)와 어긋남. 실은 cluster-manager에 `Dockerfile.ui`(별도 web-only image, WORKDIR=/app + public/)가 이미 존재 — chart는 옳은 가정이었고 ACKO Makefile이 잘못된 Dockerfile을 사용한 것이 root cause.
- **해결 결과**:
  - cluster-manager#299: `Dockerfile.ui` → `Dockerfile.web` rename (chart의 `ui.web` naming과 일관), all-in-one `Dockerfile` 제거(multi-cluster 토폴로지와 모순), `compose.yaml`을 dual-service로 split, `cd.yaml` matrix `ui` → `web` rename + `-ui` → `-web` suffix.
  - ACKO#240: chart `ui.web.image.repository` default `aerospike-cluster-manager-ui` → `aerospike-cluster-manager-web`, `Makefile [3/8]`이 `Dockerfile.api` + `Dockerfile.web` 별도 build, `[4/8]`이 양쪽 image 모두 kind load.
- **검증 완료**: clean kind cluster + chart default mountPath + `Dockerfile.web` 구조가 자동으로 일치. 7-way cross-cluster E2E 모두 통과 (common SPA → prod API, no auth 401 / health 200 / dev-user 200 / prod-user 200 / wrong-aud 401 / gibberish 401 / connections 200).

### 0.7. Go e2e 워크플로우의 SKIP_KEYCLOAK — RESOLVED
- **Status**: ✅ Resolved by aerospike-ce-kubernetes-operator#241 (2026-05-05).
- **이유**: `setup-test-e2e`가 `install-keycloak`을 unconditional 호출 → cold CI runner가 매번 bitnamilegacy/keycloak + postgresql + keycloak-config-cli pull → chart `--timeout 5m` 초과 → `context deadline exceeded`. ACKO#240 e2e CI fail의 원인.
- **해결**: Stream D가 이미 `SKIP_KEYCLOAK=true` Makefile escape hatch를 둠. e2e workflow의 `Running Test e2e` step에 `env: SKIP_KEYCLOAK: "true"` 추가. Go e2e는 OIDC 무관 (controller behaviour만 검증), OIDC + multi-cluster는 plugins repo의 `e2e_pytest/test_multi_cluster.py`에서 별도 검증.
